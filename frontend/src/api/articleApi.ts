...
export const fetchArchivedNews = async(userId: string, token: string):Promise<internalAPINew[]> =>{
  const response: internalAPINew[] = await GET_APIWithToken(articleUrlsMap.ARTICLE_GET_ARCHIVED_URL(userId), token)
  return response
}

export const deleteArchivedNewFromUser = async(userId: string, token: string, articleId:string):Promise<externalAPINew[]> =>{
  const response: externalAPINew[] = await POST_APIWithToken(articleUrlsMap.ARTICLE_POST_DELETE_URL(userId), token, {articleId})
  return response
}