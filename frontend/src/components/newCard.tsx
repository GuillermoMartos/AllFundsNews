...
function NewCard({ article, archivado = false }: NewCardProps) {
    ...
    const handleClickArchive = (articleFetched: externalAPINew | internalAPINew,  archivado:boolean=false) => {
      if(archivado){ removeArticleFromArchive(articleFetched.id) }
        else {archiveArticle(articleFetched as externalAPINew)}
  };
  ...
  return (
    <div className={styles.new_wrapper} id={article.id} key={article.id}>
              ...
              <button
                ...
                onClick={() => {handleClickArchive(article, archivado)}}
                >
                Borrar
              </button>
              : 
              <button
                className={styles.btn_archivar}
                onClick={() => {handleClickArchive(article)}}
                >
                Archivar
              </button>}
